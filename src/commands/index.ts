...
import { register as register_alphafold_2_protein_folding } from './alphafold_2_protein_folding';
import { register as register_alphafold_3_protein_folding } from './alphafold_3_protein_folding';
...
import { register as register_rfdiffusion_2_protein_design } from './rfdiffusion_2_protein_design';
import { register as register_get_tool_result } from './get_tool_result';
import { register as register_cancel_tool_call } from './cancel_tool_call';
import { register as register_upload_file_multipart } from './upload_file_multipart';
...
export function registerCommands(program: Command, baseUrl: string): void {
  ...
  register_alphafold_2_protein_folding(program, baseUrl);
  register_alphafold_3_protein_folding(program, baseUrl);
  ...
  register_rfdiffusion_2_protein_design(program, baseUrl);
  register_get_tool_result(program, baseUrl);
  register_cancel_tool_call(program, baseUrl);
  register_upload_file_multipart(program, baseUrl);
}
